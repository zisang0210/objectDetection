...
class YoloFeatureExtractorTestBase(object):
  ...
  def _validate_end_points_shape(self,
                               feature_extractor,
                               preprocessed_inputs,
                               expected_end_points_shapes):
    """Checks the feature extraction network are of correct shape.
    ...
    Args:
      ...
      expected_feature_map_shapes: The expected shape of the extracted features.
    """
    endpoints = feature_extractor.init_net_base(preprocessed_inputs)

    init_op = tf.global_variables_initializer()
    with self.test_session() as sess:
      sess.run(init_op)
      endpoints_out = sess.run(endpoints)

      self.assertItemsEqual(expected_end_points_shapes.keys(), endpoints_out.keys())
      for endpoint_name in endpoints_out:
        self.assertListEqual(list(endpoints_out[endpoint_name].shape),
                             expected_end_points_shapes[endpoint_name])
  ...
  def check_end_points_returns_correct_shape(
      self, image_height, image_width, depth_multiplier, pad_to_multiple,
      expected_end_points_shapes_out):
    feature_extractor = self._create_feature_extractor(depth_multiplier,
                                                       pad_to_multiple)
    preprocessed_inputs = tf.random_uniform(
        [4, image_height, image_width, 3], dtype=tf.float32)
    self._validate_end_points_shape(
        feature_extractor, preprocessed_inputs, expected_end_points_shapes_out)
